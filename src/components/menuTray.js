...
function MenuTray(props) {
  ...
  const [location, setLocation] = useState(window.scrollY);
  ...
  useEffect(() => {
    ...
    const scrollHandler = document.addEventListener('scroll', () => {
      ...
      updater = setTimeout(() => setLocation(window.scrollY), 100);
    });
    return () => {
      if (updater) clearTimeout(updater);
  ...
  const disclosureButtons = menuItems.map(item =>
    <DisclosureButton key={item} text={item} stowed={stowed} location={location} link={item}></DisclosureButton>);

  return <nav className={`flex flex-row inline-block p-1 ${classExtras.join(' ')}`}>
    {disclosureButtons}
  </nav>;
}
